<script>
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log('Calling authStore.signIn with:', { email, password });
      const loginResult = await authStore.signIn(email, password);
...
<svelte:head>
  <title>Test Login - Seller Portal</title>
</svelte:head>
...
<div class="min-h-screen bg-gray-50 flex items-center justify-center">
  <div class="max-w-md w-full bg-white p-8 rounded-lg shadow">
    ...
    <div class="mt-4">
      <a href="/auth/login" class="text-blue-600 hover:text-blue-800">← Back to normal login</a>
    </div>
  </div>
</div>